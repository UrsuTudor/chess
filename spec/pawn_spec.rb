...
describe Pawn do
  ...
  describe 'valid_one_forward' do
    it 'returns 2, 0 if the pawn belongs to white' do
      expect(white_pawn.valid_one_forward).to eq([2, 1])
    end

    it 'returns 5, 0 if the pawn belongs to black' do
      expect(black_pawn.valid_one_forward).to eq([5, 1])
    end
  end

  describe 'valid_double_forward' do
    it 'returns 3, 0 if the pawn belongs to white' do
      expect(white_pawn.valid_doulbe_forward).to eq([3, 1])
    end

    it 'returns 4, 0 if the pawn belongs to black' do
      expect(black_pawn.valid_doulbe_forward).to eq([4, 1])
    end
  end

  describe 'valid_white_takes' do
    it 'returns an empty array if the pawn on [1][1] belongs to white and the right diagonal is empty' do
      expect(white_pawn.valid_white_takes(board.board)).to eq([])
    end
    ...
    it 'returns 2, 2 if the pawn on [1][1] belongs to white and there is a black pawn on the right diagonal' do
      ...
      expect(white_pawn.valid_white_takes(board.board)).to eq([[2, 2]])
    end
    ...
    on both diagonals' do
      ...
      expect(white_pawn.valid_white_takes(board.board)).to eq([[2, 2], [2, 0]])
    end
  ...
  describe 'valid_black_takes' do
    ...
    it 'returns 5, 2 if the pawn on [6][1] belongs to black and there is a white pawn on the right diagonal' do
      ...
      expect(black_pawn.valid_black_takes(board.board)).to eq([[5, 2]])
    end
    ...
    on both diagonals' do
      ...
      expect(black_pawn.valid_black_takes(board.board)).to eq([[5, 2], [5, 0]])
    end
